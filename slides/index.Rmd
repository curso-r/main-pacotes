...
- **Disponibilizando seu pacote**
  - Controlando as versões com Git
  - Disponibilizando em um repositório no GitHub
  - Criando um {pkgdown}
...
- `.Rbuildignore`: .destaque[escrever!]

- `NAMESPACE`: .destaque[escrever!]


.destaque[Acho legal falar de outros tipos diretorios mais comuns, como o inst/]
...
.destaque[falar sobre o usethis::use_r()]
...
.destaque[falar sobre o usethis::use_package(), usethis::use_dev_package() e usethis::use_pipe()]
...
.destaque[falar sobre o atalho do roxygen]

---
.destaque[falar sobre documentação de funções]

.destaque[falar sobre documentação de base de dados]

.destaque[Acentos, encoding e variáveis globais]
...
.destaque[O pacote {testthat}]

.destaque[Tipos de testes]
...
.destaque[lembrete pra mim mesma: algumas funcoes mudaram. revisar. separar o que é setup do que é do fluxo de trabalho]
...
.destaque[colocar conteudo sobre Criando um {pkgdown}]

.destaque[Integração contínua com Github Actions]

.destaque[Regras para colocar um pacote no CRAN]
