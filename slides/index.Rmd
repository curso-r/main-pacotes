...
- Nós gostaríamos de saber sobre vocês (escreva no chat ✍️😀):
...
- [R instalado](https://livro.curso-r.com/1-1-instalacao-do-r.html) ✅ +  [RStudio instalado](https://livro.curso-r.com/1-1-instalacao-do-r.html) ✅

- [Ferramentas de desenvolvimento](https://r-pkgs.org/setup.html#setup-tools): ✅ Windows: [RTools instalado](https://livro.curso-r.com/1-3-instalacao-adicionais.html#rtools); Linux: [r-base-dev](https://livro.curso-r.com/1-3-instalacao-adicionais.html#rtools); macOS: Xcode command line tools.

-  Verificar se as ferramentas de desenvolvimento estão disponíveis: `devtools::has_devel()`
...
- [Git instalado](https://git-scm.com/) ✅ +  [Conta no GitHub criada](https://github.com/)  ✅


<!-- Caio, vc acha uma boa prática usar `use_devtools()`? -->
...
- Compartilhar código é sempre uma boa ideia para que a comunidade se
beneficie dos avanços individuais das pessoas que fazem parte

- Para sermos pessoas desenvolvedoras melhores, precisamos receber ajuda e
sugestões de autras desenvolvedoras mais experientes
...
<!-- slide da apresentacao https://lente.dev/packages.pdf -->
...
- O pacote `{usethis}` possui algumas funcões para nos ajudar com isso. Por exemplo:
...
.panelset[
.panel[.panel-name[Código original]
...
.panel[.panel-name[Indo além +1]

E se eu quiser que a função possibilite calcular a média para todos os meses do ano?

```{r message=FALSE, warning=FALSE, eval=FALSE}
calcular_media_volume <- function(sistema, ano, `mes = 1:12`) {
  url_mananciais <-
    "https://github.com/beatrizmilz/mananciais/raw/master/inst/extdata/mananciais.csv"
  mananciais <- readr::read_csv2(url_mananciais)
  mananciais %>%
    ...
    dplyr::group_by(sistema, ano, mes) %>%
    dplyr::summarise(media_volume_porcentagem = base::mean(volume_porcentagem)) %>%
    dplyr::filter(ano == {{ano}}, mes `%in%` {{mes}}, sistema == {{sistema}})  

} 
```
Exemplo de uso:
```{r , eval=FALSE}
calcular_media_volume(sistema = "Guarapiranga", ano = 2021)
```
...
.panel[.panel-name[Indo além +2]

E se eu quiser que a função possibilite calcular a média para todos os anos da base?

```{r message=FALSE, warning=FALSE, eval=FALSE}
calcular_media_volume <- function(sistema, `ano = 2000:2021`, mes = 1:12) {
  url_mananciais <-
    "https://github.com/beatrizmilz/mananciais/raw/master/inst/extdata/mananciais.csv"
  mananciais <- readr::read_csv2(url_mananciais)
  mananciais %>%
    ...
    dplyr::group_by(sistema, ano, mes) %>%
    dplyr::summarise(media_volume_porcentagem = base::mean(volume_porcentagem)) %>%
    dplyr::filter(ano `%in%` {{ano}}, mes %in% {{mes}}, sistema == {{sistema}})  
} 
```
Exemplo de uso:
```{r , eval=FALSE}
calcular_media_volume(sistema = "Guarapiranga")
```
...
.panel[.panel-name[Exemplo até aqui]
...
calcular_media_volume(sistema = "Guarapiranga", ano = 2021) %>% DT::datatable()
```


]
.panel[.panel-name[Mais idéias 💡]


Podemos continuar pensando em formas de melhorar nossas funções. 

Ideias para melhorar esse exemplo:

- Arredondar os resultados da média para até 2 casas decimais.

- Possibilitar calcular a média para outras variáveis.

- A importação da base é uma etapa que demora. Possibilitar que a base de dados seja armazenada no pacote, e utilizar a base do próprio pacote, diminuiria o tempo de execução da função.
...
- Copie o código disponível [neste arquivo](#) e cole no arquivo criado. .fazer[(precisamos criar o código)]
...
Boa prática: Não é aconselhável usar pontos no nome de pacotes. ([fonte](https://r-pkgs.org/workflows101.html#formal-requirements))
...
.ideia[Onde podemos falar do usethis::use_readme_rmd() + usethis::build_readme() ?]
