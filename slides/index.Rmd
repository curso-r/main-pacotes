...
- As aulas vão das 9:00 às 13:00, com uma pausa de 10 min em torno das 11:00
...
.footnote[[Nahurhodo 205 - Powerpoint é útil para a aprendizagem?](https://www.b9.com.br/shows/naruhodo/naruhodo-205-powerpoint-e-util-para-a-aprendizagem/)]
...
Evite usar letras maiúsculas ou mesmo números pois isso pode confundir os usuários. Você definitavente ganha pontos extras se conseguir inserir alguma brincadeira com a letra R no nome (`stringr`, `decryptr`, `plyr`, `purrr`, etc.).
...
1. Crie o pacote usando a função `usethis::create_package()`
...
- Podemos usar `usethis::use_r("nome_da_funcao")` para que um arquivo seja criado antes começarmos a escrever uma função.
...
Passo 5: argumentos padrão. E se eu quiser que a função possibilite calcular a média para todos os meses do ano? E se eu quiser que a função possibilite calcular a média para todos os anos da base?
...
- Crie um arquivo onde iremos escrever a função para o pacote. Dica: use a função `usethis::use_r()`!

- Copie o código disponível em [git.io/JOtMI](https://git.io/JOtMI) e cole no arquivo criado.
...
7.1 - Execute e observe o resultado de `devtools::check()`. Lá terão indicações do que devemos fazer

7.2 - Crie uma lista de variáveis globais e adicione os elementos que aparecem no check em "Undefined global functions or variables"  

7.3 - Documente a base de dados

7.4 - Documente as funções do pacote

7.5 - Execute e observe novamente o resultado de `devtools::check()`. O objetivo é obter:
...
8.1 - Use a função `usethis::use_testthat()` para preparar o pacote 

8.2 - Abra o arquivo da função que deseja escrever testes, e use a função  `usethis::use_test()` para criar o arquivo onde o teste será escrito

8.3 - Desenvolva um teste para a função

8.4 - Execute e observe novamente o resultado de `devtools::check()`. O objetivo é obter:
...
O arquivo gerado é um **RMarkdown** (`.Rmd`), e portanto precisamos converter para **Markdown** (`.md`). Para isso podemos usar a função `usethis::build_readme()`.
...
- A funcão `devtools::load_all()` simula o processo de instalação e carregamento do pacote. As funções criadas ficam disponíveis para uso. Atalho:  **Ctrl + Shift + L**.
